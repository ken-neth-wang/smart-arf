...
- **ARF assessment wizard** — guided steps with automatic Jones-criteria
  scoring and likelihood tiers. Spec:
  [docs/scoring-algorithm.md](docs/scoring-algorithm.md) · executable truth:
  [`lib/scoring.ts`](lib/scoring.ts).
- **Patient records** — search, referral-code lookup, follow-up visits,
  soft-delete with reason. Cross-clinic continuity via shared referral codes.
- **Clinic-scoped access** — health workers see their clinic's patients plus
  referrals in; admins manage clinics, users, and approvals. Sign-up is gated
  by an email allowlist.
